...
class VLMSolverModel(csdl.Model):
    ...
    def define(self):
        ...
        #
        # eval_pts_shapes = [(x[0] - 2, x[1] - 1, 3) for x in surface_shapes]
...
if __name__ == "__main__":
    ...
    eval_pts_coords = (
        (1 - eval_pts_location) * 0.5 * mesh_val[:, 0:-1, 0:-1, :] +
        (1 - eval_pts_location) * 0.5 * mesh_val[:, 0:-1, 1:, :] +
        eval_pts_location * 0.5 * mesh_val[:, 1:, 0:-1, :] +
        eval_pts_location * 0.5 * mesh_val[:, 1:, 1:, :])[0, 0, :, :]
    ...
    data = np.ones(18)
    sprs = [
        coo_array((data, (row, col)), shape=(18, 36))
    ]  #(num_evel_pts, num_total_circualtion_strength) num_total_circualtion_strength=(nx-1)*(ny-1)
