...
from UVLM_package.UVLM_system.ode_outputs import ProfileOutputSystemModel
import csdl_om
import numpy as np
...
class ODEProblemTest(ODEProblem):
    def setup(self):
        ...
        surface_wake_coords_name = surface_names[0] + '_wake_coords'
        surface_gamma_w_name = surface_names[0] + '_gamma_w'

        self.add_profile_output('L',
                                state_name=surface_gamma_w_name,
                                shape=(1, ))

        self.add_profile_output('D',
                                state_name=surface_gamma_w_name,
                                shape=(1, ))

        self.add_profile_output('C_L',
                                state_name=surface_gamma_w_name,
                                shape=(1, ))

        self.add_profile_output('C_D',
                                state_name=surface_wake_coords_name,
                                shape=(1, ))
...
class RunModel(csdl.Model):
    ...
    def define(self):
        ...
        params_dict = {
            ...
            'nt': nt,
            'free_wake': free_wake,
            'temp_fix_option': temp_fix_option
        }
        profile_params_dict = {
            'nt': nt,
            'surface_names': surface_names,
...
# frame_vel_val = np.array([-20, 0, -4])

alpha = 2
frame_vel_val = np.zeros(3)
frame_vel_val[2] = -np.sin(alpha / 180 * np.pi)
frame_vel_val[0] = -np.cos(alpha / 180 * np.pi)
