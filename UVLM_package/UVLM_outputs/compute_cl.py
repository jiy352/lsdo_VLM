...
#         mtx_val = csc_matrix((data, (rows, cols)), shape=(system_size, system_size)).toarray()
...
#         bd_vec[:, 1] = 1
#         bd_n_wake = np.concatenate((bd_coords_list[0][0, :, :, :],
#                                     wake_vortex_coords_all[-2].reshape(num_ts, -1,
#                                                                     3)[1:, :, :]))
#         rho = 0.38
        
#         sina = np.sin(alpha / 180 * np.pi)
#         cosa = np.cos(alpha / 180 * np.pi)
#         horseshoe_circulation_repeat = np.einsum('i,j->ij', horseshoe_circulation,
#                                                 np.ones(3))
...
#                             0.25 * 0.5 * mesh[1:, 0:-1, :] +
#                             0.25 * 0.5 * mesh[1:, 1:, :])
#         aic_force, _ = vel_mtx = InducedVelocity(coll_coords_list[0][0, :, :, :],
#                                                 bd_n_wake,
#                                                 normal_vecs=np.zeros(3),
#                                                 compute_aij=False)
#         induced_velocities = np.einsum("ijk,j->ik", aic_force.reshape(8, -1, 3),
